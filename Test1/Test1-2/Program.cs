...
namespace Test1_2
{
    class Program
    {
        ...
        delegate string stringingFunction(string[] a);
        static void Main(string[] args)
        {
            ...
            stringingFunction findAlphabet;
        // a label to allow us to easily loop back to the start if there are input issues
        start:
            //Need to seperate the number sorter from string sorter
            Console.WriteLine("Input \"n\" to work with numbers or \"s\" for sentences");
            string whichType = Console.ReadLine();
            whichType = whichType.ToLower();
            if (whichType.CompareTo("n") == 0)
            {
            ...
            //here is where im actually implementing the string version
            else if (whichType.CompareTo("s") == 0)
            {
                ...
                string[] aString = sInputString.Split(' ');
            ...
            ascendornot:
                ...
                if (sDirection.ToLower().StartsWith("a"))
                {
                    findAlphabet = new stringingFunction(FindLowestString);
                }
                else if (sDirection.ToLower().StartsWith("d"))
                {
                    findAlphabet = new stringingFunction(FindHighestString);
                }
                else { Console.WriteLine("input at least something that starts with a or d"); goto ascendornot; }

                int nSortedLength = 0;
                string[] aStringSorted = new string[aString.Length];
                while (aString.Length > 0)
                {
                    // store the lowest or highest unsorted value as the next sorted value
                    aStringSorted[nSortedLength] = findAlphabet(aString);

                    // remove the current sorted value
                    RemoveUnsortedString(aStringSorted[nSortedLength], ref aString);
                ...
                foreach (string thisString in aStringSorted)
                {
